...
do
    ...
    IFS="," read -r NE ND DM DF NH DS MT BS ER <<< "$i"

    # Submit the job with the appropriate variables
    qsub -q hx -v "NE=$NE,ND=$ND,DM=$DM,DF=$DF,NH=$NH,DS=$DS,MT=$MT,BS=$BS,ER=$ER" -N "MT${MT}_NE${NE}_ND${ND}_DM${DM}_DF${DF}_NH${NH}_DS${DS}_BS${BS}" /gpfs/home/ad6013/Research/CausalStructureNeuralProcess/ml2_meta_causal_discovery/experiments/causal_classification/hx1_large_model.pbs
done
