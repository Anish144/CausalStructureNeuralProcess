...
total_experiments=$(( ${#learning_rates[@]} * ${#learning_rate_decay[@]} * ${#max_epochs_list[@]} * ${#batch_sizes[@]} * ${#weight_decays[@]} ))
...
for lr in "${learning_rates[@]}"; do
  for lr_decay in "${learning_rate_decay[@]}"; do
    for epochs in "${max_epochs_list[@]}"; do
      for batch_size in "${batch_sizes[@]}"; do
        for weight_decay in "${weight_decays[@]}"; do
          # Generate a unique run name for each experiment
          experiment_name="finetune_lr${lr}_lrdecay${lr_decay}_epochs${epochs}_bs${batch_size}_wd${weight_decay}"
